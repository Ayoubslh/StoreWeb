...
function VCard({name,icon, description}: {name: string, icon: React.ReactNode, description: string}) {
    ...
    return (
        <Card className="w-[200px]     max-w-sm bg-white shadow-lg items-center hover:shadow-xl   transition duration-300 hover:-translate-y-3 ">
            <CardContent className="flex flex-col items-center justify-center p-6">
                {icon}
            </CardContent>
             <CardDescription className="flex flex-col items-center justify-center p-4">
                <CardTitle className="text-2xl font-bold sm:text-lg">{name}</CardTitle>
            </CardDescription>
